...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=dungeonchain \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=dungeond \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/dungeond ./cmd/dungeond
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/dungeond.exe ./cmd/dungeond
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/dungeond
...
draw-deps:
	...
	@goviz -i ./cmd/dungeond -d 2 | dot -Tpng -o dependency-graph.png
...
get-localic:
	...
	git clone --branch v8.6.0 https://github.com/strangelove-ventures/interchaintest.git interchaintest-downloader
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c dungeonchain --local -f chains.yaml
endif
...
set-testnet-configs:
	dungeond config set client chain-id localchain-1
	dungeond config set client keyring-backend test
	dungeond config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | dungeond keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | dungeond keys add acc1 --recover`
